...
namespace MVC.Controllers
{
    public class ContaController : Controller
    {
        ...
        public async Task<IActionResult> FinalizarCompra(string cardTrocar, string finalizar)
        {
            ...
            {
                ValorTotal = Convert.ToDouble(((decimal)final.ValorTotal).ToString("N2")),
                Qtd = final.QuantiaTotal
            };

            if (String.IsNullOrWhiteSpace(cardTrocar)) { 
                foreach (var cartao in await _cartao.ReturnCard(HttpContext.Session.GetString("IdUsuario")))
                {
        ...
        {          
            ViewBag.nomeUser = _IGeral.RetornaNomeNull(HttpContext.Session.GetString("nomeFormatado") ?? "");
            ViewBag.RNCUC = _IGeral.RetornoRNCUC(HttpContext.Session.GetString("Endereço") ?? "");
            ModelNotaFiscal? notafisc = await _notaFiscal.PegarNotaFiscal(HttpContext.Session.GetString("IdUsuario"), nota);
        ...
        public async Task<IActionResult> SeusProdutos(string botao)
        {
            ...
            switch (separador[0]) // opção
            {
                case "Deletar":
                    await _produto.DeletarProdAsync(separador[1]); // ChaveProd
                    return RedirectToAction("SeusProdutos", "Conta");
                case "Alterar":
